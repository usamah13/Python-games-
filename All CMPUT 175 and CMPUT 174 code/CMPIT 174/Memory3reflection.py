...
class Game:
            ...
            for row in self.grid:
                for tile in row:
                    if tile.select(event.pos):
                        tile.tile_exposed = True
                        if self.clicked_tile():
                            self.tile1 = tile
                            self.tile1.shownum = True
                            
                        else:
                            self.tile2 = tile
                            self.tile1.shownum = False
                            self.tile2.shownum = True
    ...
    def update(self):
        ...
        if self.tile1 is not None and self.tile2 is not None:
            time.sleep(0.5)
            self.tile1.tile_exposed = False
            self.tile2.tile_exposed = False
            self.tile1 = None
            self.tile2 = None
...
class Tile:
    ...
    def __eq__(self,other_tile):
        if other_tile != None:
            return other_tile.images == self.images    
    ...
    def draw(self):
        ...
        if self.tile_exposed:
            self.window.get_surface().blit(self.images, self.rect)
            if self.shownum:
                
                self.window.draw_string(str(self.tile_clicked), self.x, self.y)
