class SLinkedListNode:
    ...
    def setNext(self,newNext):
        self.next = newNext
class SLinkedList:
    # an instance of this class is a Single Linked List object
    ...
    def append(self,item):
        ...
        if self.size == 0: # check if list is empty
            ...
            while (current.getNext()!=None):# WHY? 
    ...
    def insert(self,pos,item):
        ...
        assert isinstance(pos,int),'positiom must be of type int'
        assert pos >= 0,'position must be a valid number in the range'
        if pos == 0:  # insert at the start of list
            self.add(item)
        elif pos >= self.size : #insert at end of list
            self.append(item)
            ...
            while (x < pos - 1):
                previousNode = previousNode.getNext() # traversing the list
                x = x + 1
            newNextNode = previousNode.getNext()

            previousNode.setNext(new_node)
            new_node.setNext(newNextNode)
            self.size = self.size +1
